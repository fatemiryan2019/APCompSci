...
//    Initialize each candidate with a variable containing their initials
    public static int MG = 0,
            ...
            WR = 0,
            HH = 0;
    public static String[] ballots = votes.split(";");

    public static void main(String[] args) {
        int numbervotes = 0;
        for (int i = 0; i < ballots.length; i++) {
            String citizen = ballots[i].substring(0, 6);
            if (isDuplicate(citizen)) {
                continue;
            }
            numbervotes++;
            String[] votes = ballots[i].split(",");
            votecounter(Integer.parseInt(votes[1]), Integer.parseInt(votes[2]), Integer.parseInt(votes[3]), Integer.parseInt(votes[4]));
        }

        System.out.println("\nPresidential Election: ");
        System.out.println("Mordy Golding: " + MG + " votes.");
        System.out.println("Shira Gotshalk: " + SG + " votes.");
        System.out.println("Stephanie Evans: " + SE + " votes.");
        System.out.println("Ray Villalobos: " + RV + " votes.");

        if (MG > SG && MG > SE && MG > RV) {
            System.out.println("Mordy Golding is your new President\n");
        } else if (SG > MG && SG > SE && SG > RV) {
            System.out.println("Shira Gotshalk is your new President\n");
        } else if (SE > MG && SE > SG && SE > RV) {
            System.out.println("Stephanie Evans is your new President\n");
        } else if (RV > MG && RV > SG && RV > SE) {
            System.out.println("Ray Villalobos is your new President\n");
        }
        System.out.println("Vice President Election: ");
        System.out.println("Dona Sarkar: " + DS + " votes.");
        System.out.println("Tyler Ahn: " + TA + " votes.");
        System.out.println("Joe Camp: " + JC + " votes.");
        System.out.println("Brandon LeBlanc received " + BL + " votes.");
        if (DS > TA && DS > JC && DS > BL) {
            System.out.println("Dona Sarkar is your new Vice President\n");
        } else if (TA > DS && TA > JC && TA > BL) {
            System.out.println("Tyler Ahn is your new Vice President\n");
        } else if (JC > DS && JC > TA && JC > BL) {
            System.out.println("Joe Camp is your new Vice President\n");
        } else if (BL > DS && BL > TA && BL > JC) {
            System.out.println("Brandon LeBlanc is your new Vice President\n");
        }

        System.out.println("Secretary Election: ");
        System.out.println("Jason Howard: " + JH + " votes.");
        System.out.println("Nikki Rubino: " + NR + " votes.");
        System.out.println("Peggy Fisher: " + PF + " votes.");
        System.out.println("Neil Edde: " + NE + " votes.");
        if (JH > NR && JH > PF && JH > NE) {
            System.out.println("Jason Howard is your new Secretary\n");
        } else if (NR > JH && NR > PF && NR > NE) {
            System.out.println("Nikki Rubino is your new Secretary\n");
        } else if (PF > JH && PF > NR && PF > NE) {
            System.out.println("Peggy Fisher is your new Secretary\n");
        } else if (NE > JH && NE > NR && NE > PF) {
            System.out.println("Neil Edde is your new Secretary\n");
        }

        System.out.println("Treasurer Election: ");
        System.out.println("David Gassner: " + DG + " votes.");
        System.out.println("Sharon Bennett: " + SB + " votes.");
        System.out.println("Walt Ritscher: " + WR + " votes.");
        System.out.println("Heather Hurley: " + HH + " votes.");

        if (DG > SB && DG > WR && DG > HH) {
            System.out.println("David Gassner is your new Treasurer\n");
        } else if (SB > DG && SB > WR && SB > HH) {
            System.out.println("Sharon Bennett is your new Treasurer\n");
        } else if (WR > DG && WR > SB && WR > HH) {
            System.out.println("Walt Ritscher is your new Treasurer\n");
        } else if (HH > DG && HH > SB && HH > WR) {
            System.out.println("Heather Hurley is your new Treasurer\n");
        }
        System.out.println("\nVotes added up to " + numbervotes + " votes.\n");

    }


    public static boolean isDuplicate(String citizen) {
        int numfound = 0;
        for (int i = 0; i < ballots.length; i++) {
            if (citizen.equals(ballots[i].substring(0, 6))) {
                numfound++;
            }
        }
    ...
    public static void votecounter(int pres, int vice, int sec, int treas) {
        if (pres == 1) {
            MG++;
        } else if (pres == 2) {
            SG++;
